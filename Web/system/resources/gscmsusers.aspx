<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="gscmsusers.aspx.cs" Inherits="Web.resources.gscmsusers" %>
                        ...
                        <div class="col-xs-12">
                            <div class="box" style="width:95%;margin-left:auto;margin-right:auto;padding:10px">
                                <div class="box-header">
                                      <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "GS Users", "add"))
                                       {%>
                                        ...
                                        <tbody>
                                            <%
                                                ...
                                                for (int i=0;i<users.Count;i++){
                                                    var p = parents.Where(x => x.id == users[i].parentId).FirstOrDefault();
                                                    string parentname = "";
                                                    if (p != null)
                                                        parentname = p.username;
                                                     %>
                                            <tr>
                                                <td><%=users[i].id %></td>
                                                ...
                                                <td><a href="#" id="parentId<%=i %>" data-type="select" data-pk="<%=users[i].id %>" data-url="resources/editRow.ashx?table=cms_user" data-title="Reporting to"><%= parentname%></a></td>
                                                ...
                                                <td><%="GS"+users[i].Zone.ZoneId+(i+1) %></td>
                                                <td><a class="btn btn-primary" class="btn btn-block btn-primary" data-toggle="modal" data-target="#type-modal<%=users[i].id %>" style="color:white;">Change User Type</a><br /><br />
                                                   <a class="btn btn-primary" class="btn btn-block btn-primary" onclick="getSubPage('subs.aspx','<%=users[i].id %>')" style="color:white;">S Users</a>

                                                    <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "GS Users", "delete"))
...
<%--<link href="//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.2/css/bootstrap.min.css" rel="stylesheet">--%>
    ...
    <script>
        $(document).ready(function () {
            ...
            <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "GS Users", "edit"))
              {%>
            <%for (int i = 0; i < users.Count; i++)
              {%>
            $('#username<%=i%>').editable();
                ...
                <div class="modal-content">
                    ...
                    <form action="#" method="post">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    <span class="input-group-addon">Reporting to:</span>
                                    <select name="selectparent" id="selectparent" class="form-control" runat="server"></select>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="input-group">
...
<%} %>
        <script type="text/javascript">
            ...
            function AddRow() {
                $.post("resources/AddRow.ashx", { table: "cms_user", Rows: $("input[name='username']").val() + "|" + $("input[name='password']").val() + "|" + $("select[name='selectstatus']").val() + "|" + $("input[name='firstname']").val() + "|" + $("input[name='lastname']").val() + "|" + $("input[name='phone']").val() + "|" + $("input[name='email']").val() + "|" + $("input[name='address']").val() + "|2|" + $("select[name='selectzone']").val() + "|" + $("select[name='selectparent']").val() },
                    function (data) {
                        if (data == "success") {
                            $(".modal-backdrop").hide();
                            getContent("gscmsusers.aspx");
