<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="webusers.aspx.cs" Inherits="Web.resources.webusers" %>
                        ...
                        <div class="col-xs-12">
                            <div class="box" style="width:95%;margin-left:auto;margin-right:auto;padding:10px">
                                <div class="box-header">
                                     <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "S Users", "add"))
                                       {%>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped" >
                                        ...
                                        <tbody>
                                            <%for(int i=0;i<results.Count;i++){
                                                    var p = parents.Where(x => x.id == results[i].parentId).FirstOrDefault();
                                                    string parentname = "";
                                                    if (p != null)
                                            ...
                                            <tr>
                                                ...
                                                <td><a href="#" id="parentId<%=i %>" data-type="select" data-pk="<%=results[i].id %>" data-url="resources/editRow.ashx?table=WebUsers" data-title="Reporting to"><%= parentname%></a></td>
                                                ...
                                                <td>
                                                    ...
                                                    <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "S Users", "delete"))
...
<%--<link href="//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.2/css/bootstrap.min.css" rel="stylesheet">--%>
    ...
    <script>
        $(document).ready(function () {
            ...
            <%if (Web.Permissions.Check(int.Parse(Request["opId"]), "S Users", "edit"))
        {%>
            <%for (int i = 0; i < results.Count; i++)
        {%>
                ...
                <div class="modal-content">
                    ...
                    <form action="#" method="post">
                        ...
                        <div class="modal-footer clearfix">
                            ...
                            <button type="button" class="btn btn-primary pull-left" onclick="AddRow()"><i class="fa fa-plus"></i> Create</button>
                        </div>
                    </form>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
...
<script>
    function AddRow() {
        $.post("resources/AddRow.ashx", { table: "WebUser", Rows: $("input[name='FirstName']").val() + "|" + $("input[name='LastName']").val()+"|"+$("input[name='PhoneNumber']").val()+"|"+$("select[name='selectzone']").val()+"|S|"+$("input[name='Username']").val()+"|"+$("input[name='Password']").val()+"|" + $("select[name='selectparent']").val() },
            function (data) {
                if(data == "success")
                {
                    $(".modal-backdrop").hide();
                    getContent("webusers.aspx");
                }
                else
                {
                    $("#textresp").html(data);
