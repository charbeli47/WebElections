<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="subgs.aspx.cs" Inherits="Web.resources.subgs" %>
<!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                        GS Users reporting to <%=parentuser!=null?parentuser.firstname+" "+parentuser.lastname:"" %>
                    </h1>
                    <%--<ol class="breadcrumb">
                                            ...
                                            <tr>
                                                ...
                                                <td><a href="#" id="address<%=i %>" data-type="text" data-pk="<%=users[i].id %>" data-url="resources/editRow.ashx?table=cms_user" data-title="Enter address line"><%=users[i].address %></a></td>
                                                    ...
                                                    <td><a class="btn btn-primary" class="btn btn-block btn-primary" data-toggle="modal" data-target="#type-modal<%=users[i].id %>" style="color:white;">Change User Type</a><br /><br />
                                                        <a class="btn btn-primary" class="btn btn-block btn-primary" onclick="getSubPage('subs.aspx','<%=users[i].id %>')" style="color:white;">S Users</a><br /><br />
    ...
    $('#submitFrm<%=user.id %>').on('submit', (function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                getSubPage('subgs.aspx', '<%=Request["pageId"] %>');
                $(".modal-backdrop").hide();
            },
            error: function (data) {
...
<%} %>
        <script type="text/javascript">
            ...
            function AddRow() {
                $.post("resources/AddRow.ashx", { table: "cms_user", Rows: $("input[name='username']").val() + "|" + $("input[name='password']").val() + "|" + $("select[name='selectstatus']").val() + "|" + $("input[name='firstname']").val() + "|" + $("input[name='lastname']").val() + "|" + $("input[name='phone']").val() + "|" + $("input[name='email']").val() + "|" + $("input[name='address']").val() + "|2|" + $("select[name='selectzone']").val() + "|<%=Request["pageId"]%>" },
                    function (data) {
                        $(".modal-backdrop").hide();
                        getSubPage('subgs.aspx', '<%=Request["pageId"] %>');
